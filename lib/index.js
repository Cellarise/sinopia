...
module.exports = function (config_hash) {
            ...
            if (config['allow_' + action](req.params.package, req.remoteUser)) {
                ...
                if (!req.remoteUser) {
                    if (req.remoteUserError) {
                        message = "can't " + action + ' restricted package, ' + req.remoteUserError;
                    } else {
                        message = "can't " + action + " restricted package without auth, did you forget 'npm set always-auth true'?";
                    }
                    next(new UError({
                        status: 403,
                        message: message
    ...
    app.get('/:package/:version?', can('access'), function (req, res, next) {
        storage.get_package(req.params.package, {req: req}, function (err, info) {
            ...
            var version = req.params.version, t;
            if (!version) {

...
Storage.prototype.update_versions = function(name, newdata, callback) {
	...
	self._read_create_package(name, function(err, data) {
        ...
        var tag, need_change;
		for (tag in newdata['dist-tags']) {
            if (newdata['dist-tags'].hasOwnProperty(tag)){
                if (!Array.isArray(data['dist-tags'][tag]) ||
                    data['dist-tags'][tag].length !== newdata['dist-tags'][tag].length) {
                    // backward compat
                    need_change = true;
                } else {
                    var i;
                    for (i=0; i<data['dist-tags'][tag].length; i++) {
        ...
        var up;
		for (up in newdata._uplinks) {
            if (newdata._uplinks.hasOwnProperty(up)){
                need_change =
	...
	self.update_package(name, function updater(data, cb) {
		...
		if (utils.is_object(metadata.dist) && typeof(metadata.dist.tarball) === 'string') {
			...
			if (utils.is_object(data._attachments[tarball])) {
				if (data._attachments[tarball].shasum !== null &&
                    metadata.dist.shasum !== null) {
	...
	return new Path_Wrapper(
		Path.join(
			Path.resolve(
				...
				this.config.get_package_setting(pkg, 'storage') || this.config.storage
			),
            pkg
		)
	);

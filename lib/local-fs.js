...
function write(dest, data, cb) {
	var safe_write = function(cb) {
		...
		fs.writeFile(tmpname, data, function(err) {
            ...
            if(fs.existsSync(dest + '.old')) {
                fs.unlinkSync(dest + '.old');
            }
...
function write_stream(name) {
	...
	fs.exists(name, function(exists) {
		if (exists) {
            return stream.emit('error', new FSError('EEXISTS'));
        }
		...
		stream.done = function() {
			function onend() {
				file.on('close', function() {
                    ...
                    if(fs.existsSync(name + '.old')) {
                        fs.unlinkSync(name + '.old');
                    }
...
function read_stream(name, stream, callback) {
	...
	var tstream = new mystreams.ReadTarballStream();
    tstream.abort = function() {
		rstream.close();
	};
	return tstream;
}
...
function create(name, contents, callback) {
	fs.exists(name, function(exists) {
		if (exists) {
            return callback(new FSError('EEXISTS'));
        }
...
function update(name, contents, callback) {
	fs.exists(name, function(exists) {
		if (!exists) {
            return callback(new FSError('ENOENT'));
        }
		write(name, contents, callback);
	});
}
		...
		fs.fstat(fd, function(err, st) {
			...
			fs.read(fd, buffer, 0, st.size, null, function(err, bytesRead, buffer) {
				...
				if (bytesRead !== st.size) {
                    return callback(new Error('st.size != bytesRead'), fd);
                }

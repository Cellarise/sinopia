...
function _setupProxy(hostname, config, mainconfig, isHTTPS) {
	...
	if (proxy_key in config) {
		this.proxy = config[proxy_key]; //jshint ignore:line
	} else if (proxy_key in mainconfig) {
		this.proxy = mainconfig[proxy_key]; //jshint ignore:line
	}
	...
	if (Array.isArray(no_proxy)) {
		for (var i=0; i<no_proxy.length; i++) {
			...
			if (no_proxy_item[0] !== '.') {
                no_proxy_item = '.' + no_proxy_item;
            }
			if (hostname.lastIndexOf(no_proxy_item) === hostname.length - no_proxy_item.length) {
				if (this.proxy) { //jshint ignore:line
					this.logger.debug({url: this.url.href, rule: no_proxy_item}, //jshint ignore:line
						'not using proxy for @{url}, excluded by @{rule} rule');
					this.proxy = false; //jshint ignore:line
				}
				break;
			}
		}
	...
	if (typeof(this.proxy) !== 'string') { //jshint ignore:line
		delete this.proxy; //jshint ignore:line
	} else {
		this.logger.debug({url: this.url.href, proxy: this.proxy}, //jshint ignore:line
...
Storage.prototype.request = function(options, cb) {
	...
	if (!this.status_check()) {
		...
		process.nextTick(function() {
			if (typeof(cb) === 'function') {
                cb(new Error('uplink is offline'));
            }
	...
	var self = this, headers = options.headers || {};
...
Storage.prototype.status_check = function(alive) {
	if (arguments.length === 0) {
		...
		if (alive) {
			...
			this.failed_requests = this.failed_requests + 1;
			if (this.failed_requests === this.max_fails) {

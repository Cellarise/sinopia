	...
	describe('race', function() {
		before(function(cb) {
			server.put_package('race', require('./lib/package')('race'), function(res, body) {
				...
				assert(body.ok.indexOf('created new package') >= 0);
				cb();
			});
		});
		...
		it('uploading 10 same versions', function(cb) {
			...
			async.parallel(fns, function(err, res) {
				...
				res.forEach(function(arr) {
					var resp = arr[0], body = arr[1];
					if (resp.statusCode === 201 && body.ok.indexOf('published') >= 0) {
                        okcount = okcount + 1;
                    }
					if (resp.statusCode === 409 && body.error.indexOf('already present') >= 0) {
                        failcount = failcount + 1;
                    }
					if (resp.statusCode === 503 && body.error.indexOf('unavailable') >= 0) {
                        failcount = failcount + 1;
                    }
				});
